...
module.exports = grammar({
  ...
  rules: {
    ...
    call_expression: $ => seq(field("function", choice($.name, $.index_expression)), "(", optional(alias($.argument_list, $.parameter_list)), ")"),
    call_body: $ =>
        seq(field("function", choice($.name, $.index_expression)), optional(seq("(", optional(alias($.argument_list, $.parameter_list)), ")")),
        seq("{", optional(seq("|", alias($.argument_list, $.parameter_list), "|")), field("body", repeat(choice($._statement, $._expression))), "}")),

    class_definition: $ => seq(repeat($._any_attribute), "class", $.name, optional(seq("is", $.name)), $.class_body),
    foreign_class_definition: $ => seq(repeat($._any_attribute), "foreign", "class", $.name, $.class_body),
    ...
    foreign_setter_definition: $ => seq("foreign", $.name, "=", "(", $.parameter, ")"),

    foreign_method_internal: $ => seq($.name, "(", optional($.parameter_list), ")"),
    ...
    foreign_constructor: $ => seq("foreign", "construct", $.foreign_method_internal),

    static_method_definition: $ => seq("static", $.method_definition),
    foreign_static_method_definition: $ => prec(1, seq("foreign", "static", $.foreign_method_internal)),

    static_getter_definition: $ => seq("foreign", "static", $.name, "=", "(", $.parameter, ")"),
    foreign_static_getter_definition: $ => prec(1, seq("foreign", "static", $.name, "=", "(", $.parameter, ")")),

    prefix_operator_definition: $ => seq(alias(choice("+", "-", "*", "/"), $.operator), field("body", $.block)),
    ...
    infix_operator_definition: $ => seq(alias(choice("+", "-", "*", "/"), $.operator), "(", $.parameter, ")", field("body", $.block)),
    ...
    _statement: $ => choice($.return_statement, $.break_statement,
        $.continue_statement, $.class_definition, $.foreign_class_definition, $.variable_definition,
        $.if_statement, $.for_statement, $.while_statement, $.import_statement,
        $.block),
    _expression: $ => choice(
      prec.left(PREC.GROUPING,     choice(seq($._expression, "[", $._expression, "]"), seq("(", $._expression, ")"))),
      prec.left(PREC.FACTOR,       choice($.raw_string, $.string, $.boolean, $.number, $.null, $.static_field, $.field, $.name, $.list, $.map, $.index_expression, $.call_expression)),
      prec.right(PREC.NEGATE,      seq(alias(choice("!", "~", "-"), $.operator), $._expression)),
      prec.left(PREC.TERM,         seq($._expression, alias(choice("*", "/", "%"), $.operator), $._expression)),
      prec.left(PREC.EXPR,         seq($._expression, alias(choice("-", "+"), $.operator), $._expression)),
      prec.left(PREC.RANGE,        seq($._expression, alias(choice("..", "..."), $.operator), $._expression)),
      prec.left(PREC.BITSHIFT,     seq($._expression, alias(choice("<<", ">>"), $.operator), $._expression)),
      prec.left(PREC.BITAND,       seq($._expression, alias("&", $.operator), $._expression)),
      prec.left(PREC.BITXOR,       seq($._expression, alias("^", $.operator), $._expression)),
      prec.left(PREC.BITOR,        seq($._expression, alias("|", $.operator), $._expression)),
      prec.left(PREC.COMPARISON,   seq($._expression, alias(choice("<", "<=", ">", ">="), $.operator), $._expression)),
      prec.left(PREC.IS,           seq($._expression, alias("is", $.operator), $._expression)),
      prec.left(PREC.EQUALS,       seq($._expression, alias(choice("==", "!="), $.operator), $._expression)),
      prec.left(PREC.AND,          seq($._expression, alias("&&", $.operator), $._expression)),
      prec.left(PREC.OR,           seq($._expression, alias("||", $.operator), $._expression)),
      prec.right(PREC.CONDITIONAL, seq($._expression, "?", $._expression, ":", $._expression )), 
      prec.right(PREC.ASSIGN,      seq(field("left", $._expression), "=", field("right", $._expression)))
    ),
  }
});
